...
from .config import get_config, layout_vars
...
def _vars_to_css(rules):
    return "".join(f"{key}:{value};" for key, value in rules.items() if value)


def build_theme_overrides():
    cfg = get_config()
    layout_css = f":root{{{_vars_to_css(layout_vars(cfg))}}}"
    js_script = "window.smwThemeConfig = window.smwThemeConfig || {};"
    return layout_css, js_script
