...
from .htmlAndCss import (bar, collapse_children_html, collapse_no_child, css,
                         deck_name, deck_option_name, end_line, gear, js,
                         number_cell, progress, start_line)
from .printing import conditionString, nowLater
from .strings import getColor, getHeader, getOverlay

debugWrongLine = debug

NEW_ORDER = ["new today", "unseen new", "new"]
DUE_ORDER = ["review today", "review", "due today", "due"]


def _normalize_column_name(conf):
    return conf.get("name", "").strip().lower()


def _column_kind(conf):
    name = _normalize_column_name(conf)
    if name in NEW_ORDER:
        return "new"
    if name in DUE_ORDER:
        return "due"
    return None


def _column_label(conf):
    """Condense the header into a compact label for the card footer."""
    kind = _column_kind(conf)
    if kind == "new":
        return "NEW"
    if kind == "due":
        return "DUE"
...
class DeckNode:
    ...
    def getNumberColumns(self, columns):
        buf = ""
        for conf in columns:
            name = conf["name"]
            if name == "new":
            ...
            kind = _column_kind(conf)
            if kind == "new":
                colour = "var(--smw-new-color)"
            elif kind == "due":
                colour = "var(--smw-due-color)"
            ...
            if contents in [0, "0", "0%", ""]:
                ...
                if whatToDo is False:
                    ...
                    colour = whatToDo
            display_value = contents
            if kind == "new":
                display_value = re.sub(r"\s*\([^)]*\)", "", str(display_value))
            else:
                display_value = str(display_value)
            label = _column_label(conf)
            buf += number_cell(colour, display_value, "", label)
        return buf
    ...
    def htmlRow(self, col, depth, cnt):
        ...
        if self.emptyRow(cnt):
            return ""
        columns = _select_primary_columns(getUserOption("columns"))
        number_columns = self.getNumberColumns(columns)
        gear_html = gear(self.did)
        card_html = f"""
    <td class = decktd>
      <div class = ios-row>
        <div class = "ios-row-card apple-glass{self.getExtraClass()}" data-did = "{self.did}">
          {self.getName(depth)}
...
def _find_matching_conf(confs, names):
    for wanted in names:
        for conf in confs:
            if not conf.get("present", True):
                continue
            if _normalize_column_name(conf) == wanted:
                return conf
    return {"name": names[0], "present": True, "absolute": True, "percent": False, "subdeck": True}


def _select_primary_columns(confs):
    new_conf = _find_matching_conf(confs, NEW_ORDER)
    due_conf = _find_matching_conf(confs, DUE_ORDER)
    return [due_conf, new_conf]
